...
import { getPromptTemplate, updatePromptTemplate } from '@/services/api'
...
export default function EditPromptPage() {
  ...
  const loadPrompt = async () => {
    try {
      const promptData = await getPromptTemplate(parseInt(params.id as string))
      ...
      setMaxTokens(promptData.maxTokens)
      setModel(promptData.model)
    } catch (err) {
      console.error('프롬프트 로딩 실패:', err)
      setError('프롬프트를 로드할 수 없습니다')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updatePromptTemplate(parseInt(params.id as string), {
        title,
      ...
      setError(err.response?.data?.message || '프롬프트 수정에 실패했습니다')
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-gray-900 tracking-tight">프롬프트 수정</h1>
        <p className="text-gray-600 mt-2">프롬프트를 수정해보세요</p>
      </div>
